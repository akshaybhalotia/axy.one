...
- **Scheduled/maintenance:** `link-check.yml` (weekly lychee sweep: builds the site to enumerate every page's links, then resolves them against the live site via `--base-url` — derived from `_config.yml` `url` — so page links, the `/short-link` redirects, and their external targets are all validated end-to-end; opens an issue on dead links. Resolving against `--base-url` rather than offline `--root-dir` is what avoids false positives on the redirects and extensionless clean-URLs); **Dependabot** (`dependabot.yml`: npm + bundler + github-actions, weekly, minor/patch grouped).
